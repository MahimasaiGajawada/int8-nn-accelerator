...
module matrix_layer_yosys #(
    ...
    logic signed [INPUT_COLS*N-1:0] dot_input;
    logic signed [INPUT_COLS*N-1:0] dot_weight;
    ...
    always_ff @( posedge clk) begin
        ...
        if(state == STORE) begin
            if(bias_result < 0) begin
                output_data[(row_index * WEIGHT_COLS + col_index) * OUTPUT_WIDTH +: OUTPUT_WIDTH] <= 0;
            end
            else begin
                output_data[(row_index * WEIGHT_COLS + col_index) * OUTPUT_WIDTH +: OUTPUT_WIDTH] <= bias_result;
            end
    ...
    for (int k = 0; k < INPUT_COLS; k++) begin
        dot_input[k*N +: N] =
            input_data_reg[(row_index * INPUT_COLS + k) * N +: N];

        dot_weight[k*N +: N] =
            weight_reg[(k * WEIGHT_COLS + col_index) * N +: N];
    end
    ...
    next_state = state;
    bias_result = $signed(dot_result) + $signed(bias_reg[col_index * ACC_WIDTH +: ACC_WIDTH]);
