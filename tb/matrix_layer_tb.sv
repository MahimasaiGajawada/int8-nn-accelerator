...
module matrix_layer_tb();

    initial begin
        $dumpfile("matrix_layer.vcd");
        $dumpvars(0, matrix_layer_tb);
    end
    ...
    matrix_layer #(.N(8), .INPUT_ROWS(2), .INPUT_COLS(2), .WEIGHT_COLS(2)) dut(.clk(clk), .rst(rst), .start(start), .input_data(input_data), .weight(weight), .bias(bias), .busy(busy), .done(done), .output_data(output_data));

    always #5 clk <= ~clk;

    task automatic test_matrix_layer(
